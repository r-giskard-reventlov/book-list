export default class BookSearch extends HTMLElement {
    ...
    constructor() {
        ...
        resultsDiv.style.width = searchField.style.width;
    ...
    resultRow = (work) => {
        ...
        return `<li class="list-group-item work" 
                    data-id="${work.key}"
                    data-name="${title}">${title}</li>`;
    };
    ...
    searchForWorks = async (ev) => {
        ...
        if(keywords.split(/\s{1,}/) === 0) return;

        await this.search.call(this, keywords)
